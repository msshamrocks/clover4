...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("CLOVERMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("CLOVERMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("CLOVERMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("clover-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running CLOVERMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
