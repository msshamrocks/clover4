...
#if defined(NDEBUG)
#error "CLOVER cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight <= 10) {
        ...
        nSubsidy = 222222 * COIN; // Premine 18% of total supply
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    if (nHeight <= 7500) {
        ...
        int64_t nMoneySupply = chainActive.Tip()->nMoneySupply;
        int64_t mNodeCoins = mnodeman.size() * GetCurrentCollateral() * COIN; // Collateral needed to run a single Shamrock Masternode
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        if ((block.nTime == (uint32_t)Params().CloverBadBlockTime()) &&
            (block.nBits == (uint32_t)Params().CloverBadBlockBits())) {
            // accept Clover block minted with incorrect proof of work threshold after pow ended!
